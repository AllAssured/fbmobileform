$(document).ready(function () {
  ...
  for (var i = 0; i < hashParams.length; i++) {
    ...
    if (fieldsValue[0] === 'first_name') {
      document.getElementById('entry_2013988063').value = decodeURIComponent(fieldsValue[1])
    } else if (fieldsValue[0] === 'last_name') {
      document.getElementById('entry_1486840883').value = decodeURIComponent(fieldsValue[1])
    } else if (fieldsValue[0] === 'email') {
      document.getElementById('entry_98673117').value = decodeURIComponent(fieldsValue[1])
    }
  }
  // NRIC number validation if its a proper number
  $('#entry_1510994699').blur(function () {
    var icNum = document.getElementById('entry_1510994699').value
    icNum = icNum.toUpperCase()
    document.getElementById('entry_1510994699').value = icNum
    console.log(icNum)
    var icArray = []
    ...
    if (icNum.length === 9) {
      ...
      if (checkSum === icArray[8]) {
        ...
        $('#entry_1510994699').removeClass('error')
        $('#entry_1510994699').addClass('valid')
      } else {
        console.log('Invalid NRIC Number')
        $('#entry_1510994699').addClass('error')
        $('#entry_1510994699').removeClass('valid')
      }
    }
  })

  // Validating of IMEI number
  $('#entry_1413818341').blur(function () {
    var imei = document.getElementById('entry_1413818341').value
    if (imei.length === 15) {
      var imeiDigit = 0
      var imeiWeight = 0
      var bEven = true
      ...
      if (lastImeiNum === checkImeiSum) {
        ...
        $('#entry_1413818341').removeClass('error')
        $('#entry_1413818341').addClass('valid')
      } else {
        console.log('IMEI is invalid')
        $('#entry_1413818341').addClass('error')
        $('#entry_1413818341').removeClass('valid')
      }
    }
  })
  ...
  var autocomplete = new google.maps.places.Autocomplete(
    (document.getElementById('entry_2081898249')),
    {types: ['geocode']}
  )
  ...
  function fillInAddress () {
    ...
    document.getElementById('entry_2081898249').value = address1
    var postalCode = place.address_components[5].long_name
    document.getElementById('entry_524144642').value = postalCode
  }
})
